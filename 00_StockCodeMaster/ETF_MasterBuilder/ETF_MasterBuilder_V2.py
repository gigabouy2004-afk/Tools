...
def extract_etf_metrics(input_filename, output_filename):
    ...
    for idx, code in enumerate(etf_codes, 1):
        ...
        if ms_session:
            fund = None
            try:
                # Try loading as a standard Fund first
                fund = ms.Funds(code, session=ms_session)
                # Test connection/validity immediately by calling a lightweight property or descriptive lookup if needed, 
                # or let it fail gracefully on the first data call.
            except Exception as e:
                error_msg = str(e)
                if "Stock should be used" in error_msg or "stock" in error_msg.lower():
                    try:
                        ...
                        print(f"  [Warning] MStarPy Stock initialization failed for {code}: {fallback_err}")
                else:
                    print(f"  [Warning] MStarPy Funds initialization failed for {code}: {e}")
            ...
            if fund:
                try:
                    ...
                    try:
                        ...
                        # Dynamic fallback inside the object if holdings fail due to type checking post-initialization
                        if "Stock should be used" in str(e):
                            try:
                                fund = ms.Stock(code, session=ms_session)
                                holdings_df = fund.holdings(holdingType='all')
                                if holdings_df is not None and not holdings_df.empty:
                    ...
                    # Extract Risk & Ratings (Alpha, Beta, Sharpe)
                    try:
                    ...
                    print(f"  [Warning] MStarPy data extraction failed for {code}: {e}")
