...
| `STATUS_QUO` | Fallback output only; base indicator context, not a ranked candidate |
...
| Extended | 35 | 40 | 25 |
| Divergence | 50 | 30 | 20 |
| Status Quo | not scored | base MACD/RSI/ADX/Bollinger values only | fallback output when no selected candidate criteria are met |
