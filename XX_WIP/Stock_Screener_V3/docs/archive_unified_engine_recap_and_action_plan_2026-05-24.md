...
The scanner has three candidate stage families:
...
The webpage also exposes `All` as a convenience selection. `All` means run all candidate stage families and display the remaining processed symbols as `STATUS_QUO`.

`Status Quo` is not a user-selected stage family. It is the output classification for symbols that do not satisfy the selected candidate-stage rules.
...
If baseline is Neutral/Mixed/Insufficient:
  output Status Quo unless selected rules provide enough evidence
```
...
Status Quo is an output classification, not a stage family and not a user-facing filter.
...
Status Quo should still explain the result and publish base indicator values. It should not be scored as a candidate family. In the v2 UI, `STATUS_QUO` rows appear only when `All` is selected.
...
- Confirm the three candidate stage families plus the `All` display mode.
...
- Status Quo fallback output: baseline + minimal explanatory values only.
...
- Results classify into at least Crossover/Divergence candidates plus `STATUS_QUO` fallback output without abusing Momentum Trading rules.
