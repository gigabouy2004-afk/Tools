...
-> stage-family layer evaluates Crossover, Divergence, Momentum Trading, and Status Quo
...
The required first-class stage families are:
...
- `L2 Stage Router`: choose the practical stage path using hardcoded rules for the four agreed families.
...
If baseline is Neutral/Mixed/Insufficient:
  classify Status Quo unless selected stage rules provide enough evidence
```
...
For now, L2 remains a hardcoded router for:
...
## 10. Stage Family: Status Quo

### Purpose

Status Quo is a first-class stage family, not merely an error state. It is used when the stock has a baseline state but does not currently satisfy the selected Crossover, Divergence, or Momentum Trading paths.
...
- `INSUFFICIENT_DATA`
- `STALE_SIGNAL`
- `NO_SELECTED_STAGE_MATCH`

Status Quo should still include a reason and key values, because a non-candidate result may be useful during market review.
...
- Status Quo is a valid stage family with an explanation.
