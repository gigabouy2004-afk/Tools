...
def fetch_etf_holdings_direct_api(etf_ticker):
    """Directly queries public finance endpoints for true ETF holdings without false institutional ownership fallbacks."""
    crumb_str = f"&crumb={YF_SESSION.crumb}" if YF_SESSION.crumb else ""
    modules_list = ['topHoldings,price', 'fundHolding,price', 'holdingPercent,price']
...
def process_single_etf(yf_etf, orig_etf, target_stocks_yf, stock_mapping):
    ...
    for yf_stock in target_stocks_yf:
        ...
        # STRICT VALIDATION: Only accept if weight is explicitly greater than 0
