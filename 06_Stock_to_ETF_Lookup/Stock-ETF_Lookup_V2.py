...
#DEFAULT_ETF_CSV = "D:\\Tools\\StockCodeMaster\\03_ETF\\24-06-US_ETF_Master_Library-filtered.csv"  # <-- Default ETF CSV path for testing
DEFAULT_ETF_CSV = ""  # <-- Default ETF CSV path for testing
FAIL_SAFE_STOCKCODE = "MU"  # <-- Default stock code for testing when no input provided
...
DEFAULT_OUTPUT_XLSX = f"StockCode_MomentumCodes_ETF_Lookup_TEST-{current_date_str}.xlsx"
...
def fetch_etf_holdings_direct_api(etf_ticker):
    """Directly queries public finance endpoints with SEC fallback validation layers."""
...
def run_etf_stock_lookup(etf_csv_path, stock_codes_str, output_xlsx_path):
    ...
    if not os.path.exists(abs_etf_path):
        ...
        return
    try:
        with open(abs_etf_path, 'r') as f: f.read(10)
    except IOError as e:
