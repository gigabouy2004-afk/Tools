...
DEFAULT_ETF_CSV = ""  # Not required; script uses built-in direct universe
FAIL_SAFE_STOCKCODE = "PLTR"  # Default stock code
TARGET_MATCH_LIMIT = 20 # Limit set for debugging/early stop
...
def get_builtin_master_etf_universe():
    """Returns a comprehensive, self-contained master list of US ETFs covering broad market, sectors, tech, and semiconductors."""
    return sorted(list(set([
        # Broad Market & Major Index ETFs
        "SPY", "QQQ", "IWM", "VTI", "VOO", "DIA", "ITOT", "SCHB", "VV", "VONE", "RSP", "QQQM", "SPLG",
        # Sector & Industry ETFs (Technology & Semiconductors where MU, NVDA, PLTR appear)
        "XLK", "SMH", "SOXX", "XSD", "PSI", "IGV", "VGT", "XLC", "XLY", "XLF", "XLE", "XLV", "XLI", "XLP", "XLU", "XLB", "XLRE",
        # Growth, Value & Factor ETFs
        "VUG", "VTV", "IWF", "IWD", "SCHG", "SCHV", "IVW", "IVE", "MTUM", "QUAL", "USMV",
        # Size & Cap ETFs
        "VB", "VO", "IWN", "IWO", "VBK", "VBR", "SCHA", "SCHM", "IWM", "IWN", "IWO",
        # Thematic, Innovation & Tech Sub-sectors
        "ARKK", "ARKW", "ARKF", "ARKQ", "ARKG", "XT", "SKYY", "CLOU", "CIBR", "PNQI", "SOCL", "BOTZ", "ROBO", "FINX", "HERO", "BUG",
        # International & Global ETFs
        "EEM", "EFA", "ACWI", "VT", "IXUS", "IEFA", "EWJ", "VGK", "EWU", "EWG", "EWQ", "EWZ", "FXI", "MCHI", "KWEB", "EWY", "EWT", "INDA", "ASHR",
        # Leveraged & Inverse ETFs
        "TQQQ", "SQQQ", "SPXL", "SPXS", "UPRO", "TZA", "FAS", "FAZ", "UVXY", "VXX", "TECL", "SOXL", "SOXS",
        # Fixed Income & Commodities
        "BND", "AGG", "LQD", "HYG", "TLT", "IEF", "SHY", "BIL", "GLD", "SLV", "DBC", "TIP", "IEF", "MUB"
    ])))

def fetch_etf_holdings_direct_api(etf_ticker):
    """Directly queries API endpoints and web page state extraction to retrieve true ETF holdings."""
    crumb_str = f"&crumb={YF_SESSION.crumb}" if YF_SESSION.crumb else ""
    modules_list = ['topHoldings,price', 'fundHolding,price']
    ...
    # Strategy 1: Official QuoteSummary API
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result:
                    ...
                    if not top_holdings:
                        top_holdings = summary.get('fundHolding', {}).get('holdings', [])
    ...
    # Strategy 2: Web Page State Extraction from Yahoo Finance holdings page
    try:
        page_url = f"https://finance.yahoo.com/quote/{etf_ticker}/holdings/"
        resp = YF_SESSION.session.get(page_url, timeout=7)
        if resp.status_code == 200:
            match = re.search(r'root\.App\.main\s*=\s*(\{.+?\});', resp.text)
            if match:
                app_data = json.loads(match.group(1))
                context = app_data.get('context', {}).get('dispatcher', {}).get('stores', {})
                quote_store = context.get('QuoteSummaryStore', {})
                
                if not ltp:
                    ltp = quote_store.get('price', {}).get('regularMarketPrice', {}).get('raw', 0.0)
                
                holdings_module = quote_store.get('topHoldings', {}).get('holdings', [])
                for h in holdings_module:
                    sym = str(h.get('symbol', '')).strip().upper()
                    w = h.get('percent', {})
                    weight = float(w.get('raw', 0.0)) if isinstance(w, dict) else float(w or 0.0)
                    if sym:
                        if 0.0 < weight < 1.0: weight *= 100
                        holdings_dict[sym] = weight
...
def process_single_etf(yf_etf, target_stocks_yf, stock_mapping):
...
def run_etf_stock_lookup(etf_csv_path, stock_codes_str, output_xlsx_path):
    ...
    if etf_csv_path and os.path.exists(os.path.abspath(etf_csv_path)):
        ...
        print("[+] Initializing direct built-in master ETF universe (Yfinance direct querying)...")
        raw_library_etfs = get_builtin_master_etf_universe()
    ...
    print(f"[+] Loaded {len(etf_list_yf)} ETF codes to scan directly via Yahoo Finance.")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_etf = {executor.submit(process_single_etf, yf_etf, target_stocks_yf, stock_mapping): yf_etf for yf_etf in etf_list_yf}
        
        for future in as_completed(future_to_etf):
            etf_read_count += 1
            if etf_read_count % 25 == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv', type=str, default=DEFAULT_ETF_CSV, help="Path to input ETF CSV reference (optional)")
