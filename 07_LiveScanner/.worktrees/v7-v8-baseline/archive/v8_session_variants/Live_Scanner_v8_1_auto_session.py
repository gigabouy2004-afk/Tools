...
# ============================================================================== 
# V8 - REFINED MOMENTUM SCANNER: SEPARATED TREND, ENTRY TIMING, LIVE CANDLE + BACKTEST
# ==============================================================================
...
def get_us_market_phase(now: datetime | None = None) -> dict:
    ...
    if not calendar.is_session(session_date):
        return {
            ...
            "reason": "Weekend or exchange holiday; use the last completed daily candle.",
        }

    session_open = calendar.session_open(session_date).tz_convert(US_MARKET_TZ)
    session_close = calendar.session_close(session_date).tz_convert(US_MARKET_TZ)
    now_ts = pd.Timestamp(now_et)
    pre_start = pd.Timestamp.combine(now_et.date(), PREMARKET_START).tz_localize(US_MARKET_TZ)
    post_end = pd.Timestamp.combine(now_et.date(), POSTMARKET_END).tz_localize(US_MARKET_TZ)

    if pre_start <= now_ts < session_open:
        phase, effective_mode, reason = "PREMARKET", "intraday", "Use today's extended-hours pre-market OHLCV candle."
    elif session_open <= now_ts < session_close:
        phase, effective_mode, reason = "REGULAR", "intraday", "Use today's pre-market plus regular-session OHLCV candle."
    elif session_close <= now_ts < post_end:
        phase, effective_mode, reason = "POSTMARKET", "intraday", "Use today's complete regular session plus post-market OHLCV to the latest print."
    else:
        phase, effective_mode, reason = "COMPLETED", "completed", "Extended-hours session is inactive; use completed daily candles."
...
def drop_incomplete_daily_row(df: pd.DataFrame, phase: str, now: datetime | None = None) -> pd.DataFrame:
    """Remove today's Yahoo daily row while pre/regular/post-market is still active."""
            ...
            if effective_mode == "intraday":
                ...
                try:
                    ...
                    if not minute_df.empty:
                        df = merge_intraday_session_candle(df, minute_df)
                    else:
                        market_context["reason"] += " Minute data unavailable; fell back to the latest completed daily candle."
                        effective_mode = "completed"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--live-candle-mode", choices=["auto", "completed", "intraday"], default="auto", help="auto=PREMARKET/REGULAR/POSTMARKET use minute OHLCV and off-hours use completed candles; completed=force completed daily candles; intraday=force extended-hours minute OHLCV")
