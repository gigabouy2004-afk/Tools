...
def evaluate_stock_momentum(
    ...
    try:
        ...
        # Default mode uses completed daily candles only. In intraday mode, rebuild
        # today's complete OHLCV candle from 1-minute data; never replace close alone.
        if as_of_date is None and live_candle_mode == "intraday":
            try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--live-candle-mode", choices=["completed", "intraday"], default="completed", help="completed=stable completed daily candles; intraday=rebuild current OHLCV candle from 1-minute data")
    ...
    columns_order = ["ticker", "timestamp", "as_of_date", "status", "message", "summary_message", "price", "ema_50", "ema_200", "macd", "macd_signal", "macd_hist", "adx", "rsi", "atr", "stoch_k", "stoch_d", "volume", "volume_avg_20", "weekly_ema_20", "weekly_ema_50", "weekly_price", "signal_score"]
