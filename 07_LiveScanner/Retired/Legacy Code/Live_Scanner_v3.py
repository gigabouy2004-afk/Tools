...
def evaluate_stock_momentum(ticker_symbol: str) -> dict:
    ...
    try:
        ...
        # Guard clause: ensure indicators didn't generate all NaNs due to shallow history
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-Day Momentum Live Scanner Engine v1.4")
    ...
    # Watchlist processing matrix with added comma separation handler
    source_message = ""
    if args.codes:
        raw_list = []
        for segment in args.codes:
            # FIXED: Splits tokens split across commas dynamically inside the parser loop
