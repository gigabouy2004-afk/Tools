...
_PERSISTENT_DAILY_CACHE = {
    "enabled": True,
    "directory": os.path.join(tempfile.gettempdir(), "LiveScannerV15", "daily"),
...
def evaluate_prepared_frame(
    ...
    *,
    ...
    """Evaluate one prepared candle view and apply the complete V15 policy."""
...
def prepare_v15_details_output(details_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if duplicates:
        raise ValueError(
            "V15 output contains duplicate case-insensitive headers: "
            + ", ".join(duplicates)
        )
    return prepared
...
if __name__ == "__main__":
    ...
    try:
        ...
        df_details_sheet = prepare_v15_details_output(
