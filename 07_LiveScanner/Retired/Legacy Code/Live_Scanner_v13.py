...
# ============================================================================== 
# VERSION 13 - CLEAN SUMMARY SHEET + DETAILS ADJUSTMENTS
# ==============================================================================
...
#DEFAULT_INPUT_CSV = "D:\\Tools\\00_StockCodeMaster\\02_Stock\\004\\004-All_US_Common_Stocks_Master_Library-Filtered_Technology.csv"
...
DEFAULT_BACKTEST_CSV = os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "V13_Live_Scanner_Backtest.csv"))
    ...
    if not macro_trend_ok:
        ...
        if len(exhaustion_reasons) >= 2:
            ...
            if adx_name == "Emerging":
                output_message = f"Buy momentum continuation#1: MACD({MACD_FAST_PERIOD},{MACD_SLOW_PERIOD},{MACD_SIGNAL_PERIOD}) is bullish with improving acceleration; ADX is emerging, so confirmation is moderate."
            elif extension_risk:
                output_message = f"Buy momentum continuation#2: MACD({MACD_FAST_PERIOD},{MACD_SLOW_PERIOD},{MACD_SIGNAL_PERIOD}) is bullish with improving acceleration; price is extended, increasing entry risk but not invalidating the move."
            else:
                output_message = f"Buy momentum continuation#3: MACD({MACD_FAST_PERIOD},{MACD_SLOW_PERIOD},{MACD_SIGNAL_PERIOD}) is bullish with improving acceleration, trend strength is confirmed, and price extension is controlled."
            ...
            output_message = "Watchlist: the negative MACD histogram is improving and a bullish MACD crossover is forming, but it has not triggered yet."
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Version 13 Momentum Scanner: Clean XLSX Output")
    ...
    if args.backtest:
        print("=" * 80, flush=True)
        print(" STARTING V11 BACKTEST", flush=True)
        print("=" * 80, flush=True)
    ...
    try:
        ...
        # ----------------------------------------------------------------------
        # GENERATE SHEET 1: "Summary" - CLEAN KEY-VALUE FORMAT
        # ----------------------------------------------------------------------
        summary_data = []
        ...
        # ----------------------------------------------------------------------
        # GENERATE SHEET 2: "Details"
        # ----------------------------------------------------------------------
        df_details_data = df_log.copy()

        target_signal_order = [
            "Buy_Momentum_extension",
            "Watchlist_Bull_Crossover_Formation",
            "Watchlist_Momentum_Building",
            "Hold",
            "Ignore_Daily_Trend",
            "Ignore_Weekly_Trend",
            "Error_Insufficient_History"
        ...
        signal_sort_map = {val.lower(): idx for idx, val in enumerate(target_signal_order)}
        
        def compute_sort_rank(sig):
            sig_str = str(sig).strip().lower()
            return signal_sort_map.get(sig_str, 999)

        df_details_data["_signal_rank"] = df_details_data["output_signal"].apply(compute_sort_rank)
        df_details_data = df_details_data.sort_values(by=["_signal_rank", "ticker"], ascending=[True, True])
        df_details_sheet = df_details_data.drop(columns=["_signal_rank"])

        # ----------------------------------------------------------------------
        # WRITE TO MULTI-SHEET EXCEL FILE WITH FORMAT ENGINE
        # ----------------------------------------------------------------------
