...
def evaluate_stock_momentum(ticker_symbol: str) -> dict:
    ...
    # 2. Extract the actual active extended hours price
        ...
        # Ch eck if we are currently in post-market or pre-market and pull those specific feeds
        ...
        # Fallback: If .info fails or is empty, extract the last tick from a 1-minute live chart
            if not live_price:
        ...
        if live_price and not pd.isna(live_price):
            ...
            print(f"Falling back to standard historical close: {df['close'].iloc[-1]}")
...
def load_tickers_from_file(csv_path: str) -> list:
    if not csv_path or not os.path.isfile(csv_path):
        print(f"ℹ[] Input file target '{csv_path}' empty or missing. Falling back to default baseline watchlist...", flush=True)
    ...
    try:
        ...
        if not found_col:
            print(f"⚠️ Error: File must contain a 'Ticker', 'Tickers' or 'Symbol' header column. Reverting to system defaults.", flush=True)
            return DEFAULT_FALLBACK_WATCHLIST
            
        # FIXED: Explicitly extraction of index [0] parses column key as flat series 
        ...
        print(f"⚠️ Error reading CSV file '{csv_path}': {str(e)}. Reverting to system defaults.", flush=True)
        return DEFAULT_FALLBACK_WATCHLIST
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-Day Momentum Live Scanner Engine v1.7")
