...
DEFAULT_INPUT_CSV = "D:\\Tools\\00_StockCodeMaster\\02_Stock\\17-07-US_Common_Stocks_Master_Library-Filtered.csv"
#DEFAULT_OUTPUT_CSV = os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "V11_Live_Scanner_Output.csv"))
DEFAULT_OUTPUT_CSV = os.path.abspath("D:/TMP/Live_Screener/V11_Live_Scanner_Output.csv")
DEFAULT_BACKTEST_CSV = os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "V11_Live_Scanner_Backtest.csv"))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Build the error extract from scanner rows only. The main output CSV is
        # written after the terminal-style post-execution summary is constructed.
        ...
        # Append the complete terminal summary as structured CSV rows. Each line
        # is stored in output_message so the file remains a valid rectangular CSV
        # and can still be loaded normally by pandas/Excel.
        blank_summary_row = {col: None for col in columns_order}
        blank_summary_row.update({
            "ticker": "POST_EXECUTION_SUMMARY",
            "status": "SUMMARY_SECTION",
            "output_signal": "POST_EXECUTION_SUMMARY",
            "output_message": "",
        })
        terminal_summary_rows = [blank_summary_row]
        for line_number, line in enumerate(final_lines, start=1):
            summary_line_row = {col: None for col in columns_order}
            summary_line_row.update({
                "ticker": "POST_EXECUTION_SUMMARY",
                "status": "SUMMARY_SECTION",
                "output_signal": f"SUMMARY_LINE_{line_number:02d}",
                "output_message": line,
                "message": line,
                "timestamp": execution_finished.strftime('%Y-%m-%d %H:%M:%S'),
                "as_of_date": final_summary.get("as_of_date"),
            })
            terminal_summary_rows.append(summary_line_row)

        output_df = pd.concat(
            [df_log, pd.DataFrame(terminal_summary_rows, columns=columns_order)],
            ignore_index=True,
        )
        output_df.to_csv(args.output, mode="w", index=False, header=True)
