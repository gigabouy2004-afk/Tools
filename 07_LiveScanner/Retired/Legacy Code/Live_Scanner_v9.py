...
# ============================================================================== 
# VERSION 9 - MARKET-STATE MOMENTUM SCANNER + DUAL BUY PATHS + AUTO SESSION
# ==============================================================================
...
DEFAULT_OUTPUT_CSV = os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "Live_Scanner_v9_Execution_log.csv"))
DEFAULT_BACKTEST_CSV = os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "Live_Scanner_v9_backtest.csv"))
...
def evaluate_frame(df: pd.DataFrame, ticker_symbol: str, config: dict) -> dict:
    ...
    frame["atr"] = ta.volatility.average_true_range(
        frame["high"], frame["low"], frame["close"], window=ATR_PERIOD
    )
    frame["stoch_k"] = ta.momentum.stoch(
        frame["high"], frame["low"], frame["close"], window=STOCH_K_PERIOD
    )
    ...
    macd_above_zero = bool(latest["macd"] > 0 and latest["macd_signal"] > 0)
    ...
    recent_hist = frame["macd_hist"].iloc[-hist_lookback:]
    macd_hist_expanding = bool(
        macd_hist_positive
        and recent_hist.iloc[-1] > recent_hist.iloc[-2]
        and recent_hist.iloc[-1] > recent_hist.mean()
    )
    macd_hist_contracting_positive = bool(
        macd_hist_positive and latest["macd_hist"] < prev["macd_hist"]
    )
    pre_bull_crossover = bool(
    ...
    trend_strength_ok = bool(
        latest["adx"] >= config.get("adx_trend_strong", BALANCED_CONFIG["adx_trend_strong"])
    )
    ...
    if current_volume is not None and volume_avg_20 is not None and volume_avg_20 > 0:
        ...
        volume_confirmed = volume_ratio >= config.get("volume_mult", BALANCED_CONFIG["volume_mult"])

    atr_value = float(latest["atr"])
    price_distance_from_ema50_atr = price_distance_from_ema50 / atr_value if atr_value > 0 else None
    price_in_safe_buy_zone = price_distance_from_ema50 <= (
        config.get("atr_safe_mult", BALANCED_CONFIG["atr_safe_mult"]) * atr_value
    )
    continuation_distance_ok = bool(
        price_distance_from_ema50 >= 0
        and price_distance_from_ema50 <= config.get("continuation_atr_mult", 2.2) * atr_value
    )
    price_hyper_extended = price_distance_from_ema50 > (
        config.get("atr_ext_mult", BALANCED_CONFIG["atr_ext_mult"]) * atr_value
    )
    ...
    stoch_crossed_up = bool(
        prev["stoch_k"] <= prev["stoch_d"] and latest["stoch_k"] > latest["stoch_d"]
    )
    stoch_recovery_trigger = bool(
        stoch_recently_oversold
        and stoch_crossed_up
        and latest["stoch_k"] <= stoch_recovery_ceiling
    )

    continuation_rsi_ok = bool(
        config.get("continuation_rsi_min", 50.0)
        <= latest["rsi"]
        <= config.get("continuation_rsi_max", 75.0)
    )
    continuation_volume_ok = bool(
        volume_ratio is None
        or volume_ratio >= config.get("continuation_volume_min_ratio", 0.80)
    )

    # V9 core correction:
    # Momentum continuation is a market state. EMA50 distance is a risk modifier,
    # not a veto. Stochastic is intentionally excluded from this pathway.
    momentum_continuation_state = bool(
        established_momentum
        and macd_hist_expanding
        and continuation_rsi_ok
        and continuation_volume_ok
        and not rsi_exhausted
    )

    metrics.update({
        "macd_state": (
            "BULL_EXPANDING" if macd_bullish and macd_hist_expanding
            else "BULL_COOLING" if macd_bullish and macd_hist_contracting_positive
        ...
        "ema50_distance_atr": round(price_distance_from_ema50_atr, 3)
        if price_distance_from_ema50_atr is not None else None,
        "output_signal": None,
        "setup_type": "NONE",
    })
    ...
    signal_score += 2 if momentum_continuation_state else 0
    signal_score += 1 if price_in_safe_buy_zone else 0

    if not macro_trend_ok:
        metrics.update({
            "status": STATUS_IGNORE,
            "output_signal": "Ignore",
            "message": MSG_NO_TREND,
    ...
    if weekly_df is not None and not weekly_trend_ok:
        metrics.update({
            "status": STATUS_IGNORE,
            "output_signal": "Ignore",
            "message": MSG_NO_WEEKLY_TREND,
            "signal_score": signal_score,
        })
        return metrics

    # Exhaustion requires at least two independent warnings. Extension alone is
    # not allowed to overwrite a valid momentum-continuation state.
    exhaustion_reasons = []
    ...
    # BUY pathway 1: pullback recovery.
    if established_momentum and price_in_safe_buy_zone and stoch_recovery_trigger:
        metrics["setup_type"] = "PULLBACK_RECOVERY"
        if volume_confirmed:
            signal_score += 2
            status = STATUS_BUY_SIGNAL
            output_signal = "Buy_Pullback_Recovery"
            message = MSG_BUY_TRIGGER
        else:
            status = STATUS_WATCHLIST
            output_signal = "Watchlist_Pullback_Recovery_But_Volume_Confirmation_Pending"
            message = "Valid pullback recovery; waiting for volume confirmation."

    # BUY pathway 2: momentum continuation. Distance changes the risk message,
    # but does not cancel the market-state qualification.
    elif momentum_continuation_state:
        signal_score += 2
        status = STATUS_BUY_SIGNAL
        output_signal = "Buy_Momentum_Extension"
        if continuation_distance_ok:
            metrics["setup_type"] = "MOMENTUM_CONTINUATION"
            message = MSG_BUY_CONTINUATION
        else:
            metrics["setup_type"] = "MOMENTUM_CONTINUATION_EXTENDED"
            distance_text = (
                f"{price_distance_from_ema50_atr:.2f} ATR"
                if price_distance_from_ema50_atr is not None else "an elevated distance"
            )
            message = (
                f"{MSG_BUY_CONTINUATION_EXTENDED} "
                f"Current distance is {distance_text} above EMA50."
            )

    elif established_momentum and stoch_recently_oversold and not stoch_crossed_up:
        metrics["setup_type"] = "PULLBACK_FORMING"
        status = STATUS_WATCHLIST
        output_signal = "Watchlist_Pullback_Forming_But_Stochastic_Reversal_Pending"
        message = "Strong trend with an oversold pullback; waiting for stochastic reversal."

    elif established_momentum and macd_hist_contracting_positive:
        metrics["setup_type"] = "MOMENTUM_COOLING"
        status = STATUS_HOLD
        output_signal = "Hold"
        message = "Bullish MACD remains above zero, but the positive histogram is contracting."

    elif established_momentum and not continuation_distance_ok:
        metrics["setup_type"] = "EXTENDED_CONTINUATION"
        status = STATUS_WATCHLIST
        output_signal = "Watchlist_Pullback_Awaited"
        ...
        metrics["setup_type"] = "PRE_BULL_CROSSOVER"
        status = STATUS_WATCHLIST
        output_signal = "Watchlist_Bull_Crossover_Formation"
        message = "MACD histogram is improving below zero; watch for an 8/21/5 bullish crossover."

    elif recovering_momentum and price_in_safe_buy_zone:
        metrics["setup_type"] = "MOMENTUM_RECOVERY"
        status = STATUS_WATCHLIST
        output_signal = "Watchlist_Momentum_Recovery"
        message = "MACD momentum is recovering near EMA50; full continuation confirmation is pending."

    elif established_momentum:
        metrics["setup_type"] = "ESTABLISHED_TREND"
        status = STATUS_HOLD
        output_signal = "Hold"
        message = MSG_HOLD_TREND

    else:
        metrics["setup_type"] = "TREND_NO_ENTRY"
        status = STATUS_HOLD
        output_signal = "Hold"
        message = "Macro trend is intact, but MACD/ADX momentum is not aligned for a fresh entry."
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Version 9 Momentum Scanner: Pullback Recovery + Momentum Continuation")
    ...
    if args.backtest:
        print("=" * 80, flush=True)
        print(" STARTING V9 BACKTEST", flush=True)
        print("=" * 80, flush=True)
    ...
    for run_number, as_of_date in enumerate(as_of_date_list, start=1):
        ...
        print("=" * 80, flush=True)
        print(f" STARTING LIVE MOMENTUM SCANNER V9 | {run_timestamp}", flush=True)
        ...
        print("\n" + "=" * 80, flush=True)
        print(f" SUMMARY EXECUTION REPORT V9 | {run_timestamp}", flush=True)
        print("=" * 80, flush=True)
        print(f" As-Of Date           : {as_of_label}", flush=True)
    ...
    columns_order = ["ticker", "timestamp", "as_of_date", "market_phase", "data_mode", "market_time_et", "data_note", "status", "output_signal", "setup_type", "message", "summary_message", "price", "ema_50", "ema_200", "ema50_distance_atr", "macd_params", "macd", "macd_signal", "macd_hist", "macd_hist_prev", "macd_state", "adx", "rsi", "atr", "stoch_k", "stoch_d", "volume", "volume_avg_20", "volume_ratio", "weekly_ema_20", "weekly_ema_50", "weekly_price", "signal_score"]
