#!/usr/bin/env python
"""
Vectorized, auditable two-year replay for the V16 scanner.
...
times per symbol and is not practical for the full U.S. + NSE master universe.
This harness calculates the same binding V16 entry predicates once per symbol,
...
import Live_Scanner_v16 as engine
...
DEFAULT_NSE_MASTER = (
    r"D:\Tools\00_StockCodeMaster\02_Stock"
    r"\22-07-NSE_Common_Stocks_Master_Library.csv"
)
DEFAULT_OUTPUT_DIR = Path("output") / "V16_Backtest_2Y_20260729"
TRADE_COLUMNS = [
    "ticker",
    "market",
    "security_name",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a vectorized two-year V16 replay over U.S. and NSE masters."
    )
    parser.add_argument("--us-master", default=DEFAULT_US_MASTER)
    parser.add_argument("--nse-master", default=DEFAULT_NSE_MASTER)
...
def load_master(path: str, market: str) -> pd.DataFrame:
    source = Path(path)
...
def load_universe(args: argparse.Namespace) -> pd.DataFrame:
    universe = pd.concat(
        [
            load_master(args.us_master, "US"),
            load_master(args.nse_master, "INDIA"),
        ],
        ignore_index=True,
    )
    ...
    if args.symbols:
        ...
        if absent:
            additions = pd.DataFrame(
                {
                    "ticker": absent,
                    "market": [
                        "INDIA" if value.endswith((".NS", ".BO")) else "US"
                        for value in absent
                    ],
                    "security_name": "",
                    "sector": "",
            ...
            universe = pd.concat([universe, additions], ignore_index=True)
...
def calculate_summary(
    ...
    return {
        "engine": "V16",
        "classification": "V16 binding BUY rules; vectorized historical replay",
        "preset": args.preset,
        ...
        "universe": {
            ...
            "india_symbols": int(universe["market"].eq("INDIA").sum()),
        ...
        "method_notes": [
            f"{args.history_period} of daily OHLCV is requested so EMA200 and 50 completed weekly candles are warm before the two-year measured window.",
            ...
            "After a BUY, the same ticker cannot enter another trade before the prior trade's exit date, matching V16 production backtest behavior.",
...
def main() -> int:
    ...
    print(
        f"V16 two-year replay: {len(universe):,} symbols, "
    ...
    trades_path = output_dir / "V16_Backtest_2Y_Trades.csv"
    coverage_path = output_dir / "V16_Backtest_2Y_Coverage.csv"
    breakdown_path = output_dir / "V16_Backtest_2Y_Breakdown.csv"
    summary_path = output_dir / "V16_Backtest_2Y_Summary.json"
