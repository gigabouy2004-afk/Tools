#!/usr/bin/env python3
"""
Momentum Engine V17.5 - Unified Pipeline with Precise MACD Zero & Signal Line Corrections
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Momentum Engine V17.5 - Unified Pipeline with Precise MACD Zero & Signal Line Filters"
    )
...
# ==========================================
# 4. UNIFIED PIPELINE ENGINE WITH MACD ZERO & SIGNAL LINE CORRECTIONS
# ==========================================
def run_unified_momentum_engine(symbols, backtest_date_str, output_path):
    print(f"[*] Initializing V17.5 pipeline for {len(symbols)} symbols (Backtest Date Control: {backtest_date_str or 'LIVE / TODAY'})...")
        ...
        try:
            ...
            if not is_true_bull:
                ...
                review_records.append({
                    "Symbol": symbol, 
                    "Status": "Filtered Out", 
                    "Message": f"Failed True Bull criteria (Price > EMA50: {price_above_ema50}, Price > EMA200: {price_above_ema200}, MACD Zero/Cross Bullish: {macd_zero_line_bullish} [MACD={val_macd:.4f}, Signal={val_signal:.4f}])"
                })
                continue
            ...
            # --- Volume Ratios (Fixed iloc slice bug) ---
    ...
    summary_records = [{
        ...
        "engine_architecture": "V17.5 Unified Pipeline (Precise MACD Zero & Signal Line Corrections)",
    ...
    print(f"[+] V17.5 execution complete. Results securely written to: {output_path}")
