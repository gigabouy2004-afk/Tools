#!/usr/bin/env python3
"""
Momentum Engine V17.20 - Precise Stochastic Crossover & Valley Validation Pipeline
...
def parse_arguments():
    print("----------------------------------------------------------------------")
    print("[*] Momentum Engine V17.20: Initializing Argument Parser & Input Resolution...")
    print("----------------------------------------------------------------------")
    
    parser = argparse.ArgumentParser(
        description="Momentum Engine V17.20 - Robust Split & Adjusted Close Audit Pipeline"
    )
...
def run_unified_momentum_engine(symbols, backtest_date_str, output_path, focus_mode="balanced", focus_top_n=50, focus_min_primary_return=0.05):
    print("----------------------------------------------------------------------")
    print(f"[*] Initializing V17.20 Momentum Pipeline for {len(symbols)} symbols...")
    ...
    summary_records = [{
        ...
        "engine_architecture": "V17.20 Precise Stochastic Crossover & Valley Validation Pipeline",
    ...
    df_etf_performance = pd.DataFrame(etf_performance_records) if etf_performance_records else pd.DataFrame(columns=[
        "ETF", "ETF Name", "As_Of_Date", "AUM_USD_M", "ETF_Price",
        "Since_Yesterday_%", "This_Week_%", "MTD_%", "Return_3M_%", "YTD_%", "Return_6M_%", "Return_1Y_%"
    ])
    if df_scoring.empty:
    ...
    if 'ETF Performance' in wb.sheetnames and wb['ETF Performance'].max_row > 1:
        ws = wb['ETF Performance']
        header_row = [cell.value for cell in ws[1]]
