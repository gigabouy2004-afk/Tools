# V16 Shadow Momentum Quality Engine

## Purpose

V16 starts from the validated V14 Enhanced positive-regime engine at commit
...
V16 then asks a separate question: is the qualifying condition strong,
...
The V16 fields are additional diagnostics only. The output explicitly records
`v16_classification_active=False`. A V16 recommendation can disagree with V14
...
V14 and V16 in parallel on the same dates and universes.
...
Important V16 fields include:

- `engine_version`, `v16_shadow_mode`, and
  `v16_classification_active`;
- `beta` for live equity and ETF rows, and `alpha` for live ETF rows;
- `v16_primary_regime_passed`;
...
V16 adds provider-supplied Beta and Alpha as descriptive listing context:
...
visible while the user scrolls through the remaining V16 diagnostics.
...
python .\Live_Scanner_v16.py `
  ...
  --live-candle-mode completed `
  -o "D:\path\v16-shadow-history.xlsx"
```
...
1. V14/V16 operational classifications have demonstrated exact parity during
   the shadow phase.
...
Absolute 20/60/120-session performance is measured in this first V16 version.
