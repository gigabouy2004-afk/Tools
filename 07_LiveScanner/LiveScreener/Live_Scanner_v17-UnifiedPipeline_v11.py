#!/usr/bin/env python3
"""
Momentum Engine V17.11 - Robust Single-Ticker Corporate Action & Split Integrity Fix
...
Sharpe/Sortino risk-adjusted ratios, and individual single-ticker auto-adjusted fetching to prevent bulk split drop bugs.
"""
...
# ==========================================
TEMP_CACHE_DIR = tempfile.mkdtemp(prefix="momentum_v1711_cache_")
...
def parse_arguments():
    print("----------------------------------------------------------------------")
    print("[*] Momentum Engine V17.11: Initializing Argument Parser & Input Resolution...")
    print("----------------------------------------------------------------------")
    
    parser = argparse.ArgumentParser(
        description="Momentum Engine V17.11 - Robust Split & Adjusted Close Audit Pipeline"
    )
...
# ==========================================
# 4. UNIFIED PIPELINE ENGINE WITH ROBUST SINGLE-TICKER FETCHING
# ==========================================
def run_unified_momentum_engine(symbols, backtest_date_str, output_path):
    print("----------------------------------------------------------------------")
    print(f"[*] Initializing V17.11 Momentum Pipeline for {len(symbols)} symbols...")
    ...
    for idx, symbol in enumerate(symbols, 1):
        try:
            # FIX: Use individual yf.Ticker().history() with auto_adjust=True 
            # to guarantee corporate action and reverse split adjustments are never dropped.
            ticker_obj = yf.Ticker(symbol)
            df = ticker_obj.history(start=start_fetch_date, end=end_fetch_date, auto_adjust=True, actions=False)
            
            if df is None or df.empty or 'Close' not in df.columns:
                failed_count += 1
                review_records.append({"Symbol": symbol, "Status": "Failed", "Message": "No historical price data returned or missing Close column"})
                continue
                
            df = df.dropna(subset=['Close']).sort_index()
            # Ensure timezone awareness is stripped for clean date comparison
            ...
            # --- Multi-Window Momentum Capture (Strict Rolling Windows) ---
            ...
            # Additional mandatory safety gate: Medium return must also be positive for true bullish momentum
    ...
    summary_records = [{
        ...
        "engine_architecture": "V17.11 Robust Single-Ticker Split Integrity Fix",
