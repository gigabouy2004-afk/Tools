#!/usr/bin/env python3
"""
Momentum Engine V17.8 - Refined Pipeline Execution Audit & Granular Summary Counts
...
multi-window historical returns, three-tiered regime classifications, strict EMA/MACD technical filters, and 
Sharpe/Sortino risk-adjusted ratios into a structured multi-tab Excel workbook with correct Read/Processed/Rejected breakdown.
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Momentum Engine V17.8 - Refined Execution Audit Summary"
    )
...
# ==========================================
# 4. UNIFIED PIPELINE ENGINE WITH REFINED SUMMARY AUDIT
# ==========================================
def run_unified_momentum_engine(symbols, backtest_date_str, output_path):
    print(f"[*] Initializing V17.8 pipeline for {len(symbols)} symbols (Backtest Date Control: {backtest_date_str or 'LIVE / TODAY'})...")
    ...
    # Audit counters based on precise user classification
    input_read_count = len(symbols)
    processed_count = 0
    evaluated_count = 0
    rejected_count = 0  # Includes insufficient history or failed technical gates (e.g. MACD / EMA filters)
    failed_count = 0    # True API/syntax/missing column execution failures (invalid stock codes)
    ...
    for symbol in symbols:
        try:
            ...
            # Symbol successfully read and processed through data structures
            processed_count += 1
            ...
            # --- Technical Overlays: EMAs & Standard EMA-based MACD ---
            ...
            # Standard Exponential Moving Average MACD (12, 26, 9)
            ...
            # --- Strict True Bull Presence Filter Conditions ---
            price_above_ema50 = end_price > ema_50
            price_above_ema200 = end_price > ema_200
            
            # Mandatory MACD Requirements:
            # 1. MACD Line & Signal Line are BOTH above Zero Line (val_macd > 0 and val_signal > 0)
            # 2. MACD Line is Above Signal Line (val_macd > val_signal) -> Positive Histogram
            macd_zero_line_bullish = bool(val_macd > 0 and val_signal > 0 and val_macd > val_signal)
            
            is_true_bull = price_above_ema50 and price_above_ema200 and macd_zero_line_bullish
            ...
            if not is_true_bull:
                ...
                review_records.append({
                    "Symbol": symbol, 
                    "Status": "Rejected", 
                    "Message": f"Failed True Bull technical gate (Price > EMA50: {price_above_ema50}, Price > EMA200: {price_above_ema200}, MACD Zero/Cross Bullish: {macd_zero_line_bullish} [MACD={val_macd:.4f}, Signal={val_signal:.4f}])"
                })
                continue
    ...
    # --- Refined Execution Run Summary Table ---
    summary_records = [{
        ...
        "engine_architecture": "V17.8 Refined Execution Audit Summary",
    ...
    # Fallback to prevent empty DataFrame openpyxl errors if all symbols are rejected/filtered out
    df_summary = pd.DataFrame(summary_records)
    df_review = pd.DataFrame(review_records) if review_records else pd.DataFrame(columns=["Symbol", "Status", "Message"])
    df_baseline = pd.DataFrame(baseline_records) if baseline_records else pd.DataFrame(columns=["Symbol", "Baseline_Start_Date", "Baseline_Start_Price", "Current_Date", "Current_Price", "Total_Sessions_Available"])
    df_technical = pd.DataFrame(technical_records) if technical_records else pd.DataFrame(columns=["Symbol", "Current_Price", "As_Of_Date", "MACD_Line", "MACD_Signal", "MACD_Zero_Line_Bullish"])
    if df_scoring.empty:
        df_scoring = pd.DataFrame(columns=["Symbol", "Last Traded Price", "Primary_Evaluation_Window", "Primary_Return", "Regime_Tier", "Action_Bias", "Sharpe_Ratio", "Sortino_Ratio", "MACD_Zero_Line_Bullish"])
    ...
    if 'Scoring' in wb.sheetnames and wb['Scoring'].max_row > 1:
        ...
        for col_name, fmt in [('Primary_Return', '0.00%'), ('Last Traded Price', '0.00'), ('Sharpe_Ratio', '0.00'), ('Sortino_Ratio', '0.00')]:
    ...
    print(f"[+] V17.8 execution complete. Results securely written to: {output_path}")
