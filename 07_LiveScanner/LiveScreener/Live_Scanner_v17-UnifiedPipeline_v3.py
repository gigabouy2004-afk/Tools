...
def run_unified_momentum_engine(symbols, backtest_date_str, output_path):
    ...
    # --- Post-Processing Scoring DataFrame for Tier Grouping & Primary Return Sorting ---
    ...
    if not df_scoring.empty:
        ...
        df_scoring = df_scoring.sort_values(by=['Tier_Sort', 'Primary_Return'], ascending=[True, False]).drop(columns=['Tier_Sort'])
    ...
    # --- Apply Excel Number Formatting to Scoring Sheet ---
    ...
    if 'Scoring' in wb.sheetnames:
        ...
        # Format Last Traded Price to restrict to 2 decimal places
