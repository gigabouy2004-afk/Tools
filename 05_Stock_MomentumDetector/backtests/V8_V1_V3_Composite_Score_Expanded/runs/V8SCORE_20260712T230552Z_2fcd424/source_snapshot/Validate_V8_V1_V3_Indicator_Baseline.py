...
def experiment_stock_sector_pairs(experiment):
    if "Sector_Stocks" in experiment:
        return [
            (ticker, sector)
            for sector, tickers in experiment["Sector_Stocks"].items()
            for ticker in tickers
        ]
    return [(ticker, "") for ticker in experiment["Seed_Stocks"]]
...
def validate(run_dir, skip_checksums=False):
    ...
    stock_sector_pairs = experiment_stock_sector_pairs(experiment)
    profiles = experiment.get("Profiles", list(config["Profiles"]))
    expected_rows = (
        len(stock_sector_pairs)
    ...
    independent_rows = []
    for ticker, sector in stock_sector_pairs:
        ...
        for signal_date in experiment["Signal_Dates"]:
            for profile in profiles:
                ...
                if sector and actual.get("Sector", "") != sector:
                    failures.append(f"sector mismatch {ticker}: {actual.get('Sector', '')} != {sector}")
