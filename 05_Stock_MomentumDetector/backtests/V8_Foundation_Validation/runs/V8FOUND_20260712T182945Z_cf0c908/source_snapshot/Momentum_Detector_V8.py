...
REJECT_SCORE_CAP = 49
DEFAULT_MACD_FAST_PERIOD = 8
DEFAULT_MACD_SLOW_PERIOD = 21
DEFAULT_MACD_SIGNAL_PERIOD = 5
DEFAULT_FOUNDATION_POLICY = "enforce"
...
CSV_FIELDS = [
    ...
    "Foundation_Status", "Foundation_Qualified", "Foundation_Reason", "Foundation_Policy",
    ...
    "MACD_Line", "MACD_Signal_Line", "MACD_Histogram", "MACD_Bullish_Positive",
...
def build_foundation_status_row(ticker, row, foundation_policy=DEFAULT_FOUNDATION_POLICY):
    status = row.get("Foundation_Status", FOUNDATION_INSUFFICIENT_DATA)
    reason = row.get("Foundation_Reason", "foundation indicators unavailable")
    if status == FOUNDATION_MACD_RESET:
        long_term_status = "Watchlist Candidate"
        entry_timing_status = "Foundation - MACD Reset"
    else:
        long_term_status = "Avoid"
        entry_timing_status = (
            "Insufficient history"
            if status == FOUNDATION_INSUFFICIENT_DATA
            else "Foundation - Below EMA200"
        )
    output = build_status_row(ticker, long_term_status, entry_timing_status, reason)
    output.update(
        {
            "Foundation_Status": status,
            "Foundation_Qualified": bool(row.get("Foundation_Qualified", False)),
            "Foundation_Reason": reason,
            "Foundation_Policy": str(foundation_policy).upper(),
            "Score_Message": "Setup/Momentum analysis skipped: foundation did not qualify.",
        }
    )
...
def validate_macd_periods(fast_period, slow_period, signal_period):
    periods = (int(fast_period), int(slow_period), int(signal_period))
...
def calculate_macd(
    close,
    fast_period=DEFAULT_MACD_FAST_PERIOD,
    slow_period=DEFAULT_MACD_SLOW_PERIOD,
    signal_period=DEFAULT_MACD_SIGNAL_PERIOD,
):
    fast_period, slow_period, signal_period = validate_macd_periods(
    ...
    return pd.DataFrame(
        {
            ...
            "MACD_Bullish_Positive": (macd_line > signal_line) & (macd_line > 0),
        },
        index=close.index,
    )


def calculate_foundation_indicators(
    df,
    ...
    """Calculate only the indicators required by the first V8 qualification stage."""
    df = df.copy()
    close = df["Close"]
    df["EMA_200"] = ema(close, 200)
    macd_fast_period, macd_slow_period, macd_signal_period = validate_macd_periods(
        macd_fast_period, macd_slow_period, macd_signal_period
    )
    macd = calculate_macd(
        close,
    ...
    for field in macd.columns:
        df[field] = macd[field]
    df["MACD_Fast_Period"] = macd_fast_period
    df["MACD_Slow_Period"] = macd_slow_period
    df["MACD_Signal_Period"] = macd_signal_period

    sufficient = df[["Close", "EMA_200", "MACD_Line", "MACD_Signal_Line"]].notna().all(axis=1)
    above_ema200 = sufficient & (df["Close"] > df["EMA_200"])
    bullish_positive = above_ema200 & df["MACD_Bullish_Positive"].fillna(False).astype(bool)
    df["Foundation_Qualified"] = bullish_positive
    df["Foundation_Status"] = FOUNDATION_INSUFFICIENT_DATA
    df.loc[sufficient & ~above_ema200, "Foundation_Status"] = FOUNDATION_BELOW_EMA200
    df.loc[above_ema200 & ~bullish_positive, "Foundation_Status"] = FOUNDATION_MACD_RESET
    df.loc[bullish_positive, "Foundation_Status"] = FOUNDATION_VALID
    df["Foundation_Reason"] = "foundation indicators unavailable"
    df.loc[sufficient & ~above_ema200, "Foundation_Reason"] = "close at or below EMA200"
    df.loc[above_ema200 & ~bullish_positive, "Foundation_Reason"] = (
        "close above EMA200 but MACD is not above both signal and zero"
    )
    df.loc[bullish_positive, "Foundation_Reason"] = (
        "close above EMA200 and MACD above both signal and zero"
    )
    return df
...
def calculate_v5_indicators(
    ...
    df = calculate_foundation_indicators(
        df,
...
def build_output_row(
    ...
    long_term_status,
    reason,
    foundation_policy=DEFAULT_FOUNDATION_POLICY,
):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            if df.empty:
                ...
                foundation_df = calculate_foundation_indicators(
                    df,
                    macd_fast_period=args.macd_fast,
                    macd_slow_period=args.macd_slow,
                    macd_signal_period=args.macd_signal,
                )
                foundation_row = foundation_df.iloc[-1]
                if (
                    args.foundation_policy == "enforce"
                    and foundation_row["Foundation_Status"] != FOUNDATION_VALID
                ):
                    output = build_foundation_status_row(
                        ticker, foundation_row, args.foundation_policy
                    )
                else:
...
def main():
    ...
    parser.add_argument("--macd-fast", type=int, default=DEFAULT_MACD_FAST_PERIOD,
                        help="Foundation MACD fast EMA period.")
    parser.add_argument("--macd-slow", type=int, default=DEFAULT_MACD_SLOW_PERIOD,
                        help="Foundation MACD slow EMA period.")
    parser.add_argument("--macd-signal", type=int, default=DEFAULT_MACD_SIGNAL_PERIOD,
                        help="Foundation MACD signal EMA period.")
    parser.add_argument(
        "--foundation-policy",
        choices=["enforce", "audit"],
        default=DEFAULT_FOUNDATION_POLICY,
        help="Enforce the foundation first filter, or calculate it without changing legacy decisions.",
    )
    ...
    try:
        validate_macd_periods(args.macd_fast, args.macd_slow, args.macd_signal)
