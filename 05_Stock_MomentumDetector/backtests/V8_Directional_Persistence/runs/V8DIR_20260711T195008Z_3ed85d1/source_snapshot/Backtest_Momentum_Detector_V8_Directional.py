...
def main():
    ...
    focus_results = []
    focus_ticker = str(config.get("Focus_Ticker_First_Active", "")).strip().upper()
    if focus_ticker:
        focus_candidates = execution.loc[
            execution["Ticker"].eq(focus_ticker)
            & execution["Final_Decision"].eq("MOMENTUM_ACTIVE")
        ].sort_values("Signal_Date")
        primary_candidates = focus_candidates.loc[
            focus_candidates["Primary_Episode"].eq(True)
        ]
        if not primary_candidates.empty:
            focus_candidates = primary_candidates
        if focus_candidates.empty:
            raise RuntimeError(f"no Active row found for focus ticker {focus_ticker}")
        focus_source = focus_candidates.iloc[0]
        focus_date = pd.Timestamp(focus_source["Signal_Date"])
        focus_price = load_price(source_run / f"inputs/prices/{focus_ticker}.csv")
        focus_result = {
            "Run_ID": run_id,
            "Ticker": focus_ticker,
            "Signal_Date": focus_date.date().isoformat(),
            "Final_Decision": focus_source["Final_Decision"],
            "Score": focus_source["Score"],
            "Final_Decision_Reason": focus_source["Final_Decision_Reason"],
            **directional_metrics(focus_price, focus_date, horizons),
        }
        focus_results.append(focus_result)
        write_csv(focus_results, outputs_dir / "focus_ticker_first_active_result.csv")
        log(
            f"FOCUS_FIRST_ACTIVE ticker={focus_ticker} date={focus_date.date()} "
            f"d1={focus_result['D1_Close_vs_D_Close_Pct']:.2f}% "
            f"d5={focus_result['D5_Close_vs_D_Close_Pct']:.2f}% "
            f"d8={focus_result['D8_Close_vs_D_Close_Pct']:.2f}%"
        )
    ...
    summary = {
        ...
        "Focus_Ticker_First_Active": focus_ticker or None,
        "Focus_Ticker_First_Active_Date": focus_results[0]["Signal_Date"] if focus_results else None,
        "Focus_Ticker_D1_Close_vs_D_Close_Pct": focus_results[0]["D1_Close_vs_D_Close_Pct"] if focus_results else None,
        "Focus_Ticker_D5_Close_vs_D_Close_Pct": focus_results[0]["D5_Close_vs_D_Close_Pct"] if focus_results else None,
        "Focus_Ticker_D8_Close_vs_D_Close_Pct": focus_results[0]["D8_Close_vs_D_Close_Pct"] if focus_results else None,
