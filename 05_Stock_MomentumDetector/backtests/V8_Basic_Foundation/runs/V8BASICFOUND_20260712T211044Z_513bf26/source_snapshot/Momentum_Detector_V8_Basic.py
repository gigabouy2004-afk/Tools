...
ENGINE_VERSION = "V8_BASIC_FOUNDATION_2"
...
def calculate_indicator_frame(price_df, config):
    """Calculate raw V1-V3 indicators without assigning scores, gates, or decisions."""
    frame = calculate_foundation_frame(price_df, config)
    high = frame["High"]
    low = frame["Low"]
    close = frame["Close"]
    volume = frame["Volume"]
...
def evaluate_foundation(ticker, price_df, config, as_of=None, evaluated_at=None):
    ...
    if not bars_used:
        ...
        calculated = calculate_indicator_frame(frame, config)
