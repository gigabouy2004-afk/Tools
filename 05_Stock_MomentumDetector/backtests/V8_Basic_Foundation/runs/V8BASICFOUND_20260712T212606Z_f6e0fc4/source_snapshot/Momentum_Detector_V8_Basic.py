...
ENGINE_VERSION = "V8_BASIC_FOUNDATION_4"
...
def load_config(config_path=DEFAULT_CONFIG_PATH, overrides=None):
    ...
    if config.get("Decision_Scope") != "FOUNDATION_THEN_RSI_CONTINUATION_NO_SCORE":
        raise ValueError(
            "basic configuration must use FOUNDATION_THEN_RSI_CONTINUATION_NO_SCORE"
        )
    if config.get("Indicator_Authority") != "RSI_GATE_OTHER_CALCULATION_ONLY":
        raise ValueError(
            "basic indicator layer must use RSI_GATE_OTHER_CALCULATION_ONLY authority"
        )
    ...
    if rsi_rule.get("Authority") != "CONTINUATION_GATE":
        raise ValueError("RSI Authority must be CONTINUATION_GATE")
