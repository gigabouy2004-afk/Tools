...
class V8BasicFoundationTests(unittest.TestCase):
    ...
    def test_basic_output_contains_no_score_benchmark_or_etf_fields(self):
        ...
        self.assertEqual(
            result["Decision_Scope"], "FOUNDATION_THEN_CONFIGURABLE_RSI_NO_SCORE"
        )
        self.assertEqual(
            result["Indicator_Authority"],
            "CONFIGURABLE_RSI_GATE_OTHER_CALCULATION_ONLY",
        )
        self.assertEqual(
            result["RSI_Authority"], "CONFIGURABLE_CONTINUATION_GATE"
        )
        self.assertEqual(result["RSI_Rule_Type"], "LOWER_UPPER_RANGE")
        self.assertEqual(
            result["RSI_Limit_Status"], "PLACEHOLDER_FUNCTIONAL_TEST_ONLY"
        )
        self.assertFalse(result["RSI_Operational_Use_Approved"])
    ...
    def test_placeholder_limits_cannot_claim_operational_approval(self):
        config = json.loads(basic.DEFAULT_CONFIG_PATH.read_text(encoding="utf-8"))
        config["Indicator_Rules"]["RSI"]["Operational_Use_Approved"] = True
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / "invalid_config.json"
            path.write_text(json.dumps(config), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "true only for"):
