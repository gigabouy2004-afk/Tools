...
ENGINE_VERSION = "V8_BASIC_FOUNDATION_5"
...
RSI_NOT_EVALUATED = "RSI_NOT_EVALUATED_FOUNDATION_INELIGIBLE"
RSI_WITHIN_RANGE = "RSI_WITHIN_CONFIGURED_RANGE"
RSI_BELOW_RANGE = "RSI_BELOW_CONFIGURED_RANGE"
RSI_ABOVE_RANGE = "RSI_ABOVE_CONFIGURED_RANGE"
RSI_UNAVAILABLE = "RSI_VALUE_UNAVAILABLE"
...
def load_config(config_path=DEFAULT_CONFIG_PATH, overrides=None):
    ...
    config["Indicator_Rules"]["RSI"] = dict(config["Indicator_Rules"]["RSI"])
    for key, value in (overrides or {}).items():
        ...
        elif key.startswith("RSI_Rule."):
            config["Indicator_Rules"]["RSI"][key.split(".", 1)[1]] = float(value)
        else:
            config[key] = value
    validate_periods(
        config["EMA_Period"],
    ...
    if config.get("Decision_Scope") != "FOUNDATION_THEN_CONFIGURABLE_RSI_NO_SCORE":
        raise ValueError(
            "basic configuration must use FOUNDATION_THEN_CONFIGURABLE_RSI_NO_SCORE"
        )
    if config.get("Indicator_Authority") != "CONFIGURABLE_RSI_GATE_OTHER_CALCULATION_ONLY":
        raise ValueError(
            "basic indicator layer must use CONFIGURABLE_RSI_GATE_OTHER_CALCULATION_ONLY authority"
        )
    ...
    lower = float(rsi_rule["Lower_Limit"])
    upper = float(rsi_rule["Upper_Limit"])
    if not (math.isfinite(lower) and math.isfinite(upper) and lower < upper):
        raise ValueError("RSI lower limit must be finite and smaller than upper limit")
    if rsi_rule.get("Boundary_Mode") != "INCLUSIVE":
        raise ValueError("RSI Boundary_Mode must be INCLUSIVE")
    if rsi_rule.get("Rule_Type") != "LOWER_UPPER_RANGE":
        raise ValueError("RSI Rule_Type must be LOWER_UPPER_RANGE")
    if rsi_rule.get("Authority") != "CONFIGURABLE_CONTINUATION_GATE":
        raise ValueError("RSI Authority must be CONFIGURABLE_CONTINUATION_GATE")
    valid_limit_statuses = {
        "PLACEHOLDER_FUNCTIONAL_TEST_ONLY",
        "RESEARCH_CANDIDATE_NOT_OPERATIONAL",
        "USER_APPROVED_OPERATIONAL",
    }
    limit_status = rsi_rule.get("Limit_Status")
    if limit_status not in valid_limit_statuses:
        raise ValueError(f"unsupported RSI Limit_Status: {limit_status!r}")
    operational_approved = rsi_rule.get("Operational_Use_Approved")
    if not isinstance(operational_approved, bool):
        raise ValueError("RSI Operational_Use_Approved must be Boolean")
    if operational_approved != (limit_status == "USER_APPROVED_OPERATIONAL"):
        raise ValueError(
            "RSI Operational_Use_Approved must be true only for USER_APPROVED_OPERATIONAL"
        )
...
def evaluate_rsi_value(rsi_value, lower_limit, upper_limit):
    """Evaluate an RSI value against inclusive configured continuation limits."""
    ...
    if rsi_value is None or pd.isna(rsi_value):
        return {
            "RSI_Range_Status": RSI_UNAVAILABLE,
            "RSI_Allows_Further_Processing": False,
            "RSI_Message": "RSI value is unavailable; further indicator rule processing is not allowed",
        }
    value = float(rsi_value)
    if value < lower:
        return {
            "RSI_Range_Status": RSI_BELOW_RANGE,
            "RSI_Allows_Further_Processing": False,
            "RSI_Message": (
    ...
    if value > upper:
        return {
            "RSI_Range_Status": RSI_ABOVE_RANGE,
            "RSI_Allows_Further_Processing": False,
            "RSI_Message": (
    ...
    return {
        "RSI_Range_Status": RSI_WITHIN_RANGE,
...
def evaluate_foundation(ticker, price_df, config, as_of=None, evaluated_at=None):
    ...
    base = {
        ...
        "RSI_Rule_Type": config["Indicator_Rules"]["RSI"]["Rule_Type"],
        "RSI_Limit_Status": config["Indicator_Rules"]["RSI"]["Limit_Status"],
        "RSI_Operational_Use_Approved": config["Indicator_Rules"]["RSI"][
            "Operational_Use_Approved"
        ],
        "RSI_Lower_Limit": float(
            config["Indicator_Rules"]["RSI"]["Lower_Limit"]
        ),
        "RSI_Upper_Limit": float(
            config["Indicator_Rules"]["RSI"]["Upper_Limit"]
        ),
        "RSI_Range_Status": RSI_NOT_EVALUATED,
        "RSI_Allows_Further_Processing": "",
        "RSI_Message": (
            "RSI rule was not evaluated because Foundation eligibility was not confirmed"
        ),
...
def parse_args():
    ...
    parser.add_argument("--rsi-lower-limit", type=float)
    parser.add_argument("--rsi-upper-limit", type=float)
...
def main():
    ...
    overrides = {
        ...
        "RSI_Rule.Lower_Limit": args.rsi_lower_limit,
        "RSI_Rule.Upper_Limit": args.rsi_upper_limit,
    ...
    for ticker in list(dict.fromkeys(value.strip().upper() for value in args.tickers if value.strip())):
            ...
            row.update(
                {
                    ...
                    "RSI_Lower_Limit": config["Indicator_Rules"]["RSI"]["Lower_Limit"],
                    "RSI_Upper_Limit": config["Indicator_Rules"]["RSI"]["Upper_Limit"],
                    "RSI_Range_Status": "RSI_NOT_EVALUATED_DATA_ERROR",
                    "RSI_Message": "RSI rule was not evaluated because price data failed",
        ...
        print(
            ...
            f"reason={row['Foundation_Reason']} | rsi={row['RSI']} | "
            f"rsi_status={row['RSI_Range_Status']} | rsi_message={row['RSI_Message']}"
        )
