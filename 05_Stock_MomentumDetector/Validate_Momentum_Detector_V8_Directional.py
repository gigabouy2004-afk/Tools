...
def validate(run_dir, skip_checksums=False):
    ...
    focus_path = run_dir / "outputs/focus_ticker_first_active_result.csv"
    if focus_path.exists():
        focus_rows = list(csv.DictReader(open(focus_path, encoding="utf-8")))
        if len(focus_rows) != 1:
            failures.append(f"expected one focus row, found {len(focus_rows)}")
        for row in focus_rows:
            ticker = row["Ticker"]
            frame = load_price(source_run / f"inputs/prices/{ticker}.csv")
            expected = expected_metrics(frame, row["Signal_Date"])
            for field, expected_value in expected.items():
                actual_text = row[field]
                if isinstance(expected_value, bool):
                    passed = actual_text.lower() == str(expected_value).lower()
                ...
                if not passed:
                    failures.append(f"focus {ticker} mismatch: {field}")
    validation_dir = run_dir / "validation"
    with open(validation_dir / "independently_recomputed_metrics.csv", "w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=["Scope", "Ticker", "Field", "Expected", "Actual", "Pass"])
        writer.writeheader()
