...
DEFAULT_BASELINE_RUN = (
    DEFAULT_OUTPUT_ROOT / "V8BASICFOUND_20260712T204114Z_36c2e24"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Frozen replay of the V8 Foundation-first indicator base layer"
    )
...
def main():
    ...
    for stock in stocks:
        ...
        for signal_date in dates:
            ...
            log(
                ...
                f"indicator_module={result['Indicator_Module_Status']} "
                f"rsi_status={result['RSI_Range_Status']} "
    ...
    if DEFAULT_BASELINE_RUN.is_dir():
        baseline = pd.read_csv(
            DEFAULT_BASELINE_RUN / "outputs" / "foundation_results.csv", dtype=str
        ).fillna("")
        baseline_index = {
        ...
        for row in rows:
            ...
            prior = baseline_index.get(key, {})
            changed = []
            for field in comparison_fields:
                current_value = str(row.get(field, ""))
                prior_value = str(prior.get(field, ""))
                if field in {
                    "Close",
                    "EMA_Value",
                    "MACD_Line",
                    ...
                    try:
                        matches = abs(float(current_value) - float(prior_value)) <= 1e-9
                    except (TypeError, ValueError):
                        matches = current_value == prior_value
                else:
                    matches = current_value.lower() == prior_value.lower()
                if not matches:
                    changed.append(field)
            foundation_regression.append(
                {
                    "Ticker": key[0],
                    "As_Of_Date": key[1],
                    "Regression_Status": "PASS" if not changed else "FAIL",
                    "Changed_Fields": ";".join(changed),
                }
            )
        write_csv(foundation_regression, outputs_dir / "foundation_regression.csv")
    ...
    aggregate = {
        ...
        "Indicator_Set_ID": basic_config["Indicator_Set_ID"],
        "Indicator_Authority": basic_config["Indicator_Authority"],
        "RSI_Limit_Status": basic_config["Indicator_Rules"]["RSI"]["Limit_Status"],
        "RSI_Operational_Use_Approved": basic_config["Indicator_Rules"]["RSI"][
        ...
        "Indicator_Module_Executed_Rows": sum(
            row["Indicator_Module_Status"] == basic.INDICATOR_MODULE_EXECUTED
            for row in rows
        ),
        "Indicator_Module_Not_Run_Rows": sum(
            row["Indicator_Module_Status"] == basic.INDICATOR_MODULE_NOT_RUN
            for row in rows
        ),
        "RSI_Allows_Further_Processing_Rows": sum(
            row["RSI_Allows_Further_Processing"] is True for row in rows
        ),
        "RSI_Stops_Further_Processing_Rows": sum(
            row["RSI_Allows_Further_Processing"] is False for row in rows
        ),
        "RSI_Status_Counts": {
            status: sum(row["RSI_Range_Status"] == status for row in rows)
            for status in sorted({row["RSI_Range_Status"] for row in rows})
        },
        "State_Counts": {
        ...
        "Forbidden_Output_Fields_Present": sorted(
            set(rows[0]).intersection(
                {"Score", "Benchmark_Ticker", "ETF_Ticker", "Weekly_Trend"}
            )
        ),
        "Foundation_Regression_Rows": len(foundation_regression),
        "Foundation_Regression_Failures": sum(
            row["Regression_Status"] != "PASS" for row in foundation_regression
        ),
    }
    ...
    manifest = {
        **aggregate,
        ...
        "Known_Limitations": [
            ...
            "This run functionally validates the Foundation-to-RSI sequence; it does not establish that RSI 30-65 improves trading outcomes.",
            "ADX, ATR, OBV and Aroon remain calculation-only and have no configured continuation rule.",
        ],
    }
    ...
    (run_dir / "README.md").write_text(
        f"# {run_id}\n\nV8 EMA/MACD Foundation-first frozen replay with an inclusive RSI 30-65 continuation gate. ADX, ATR, OBV and Aroon remain calculation-only. No Score, benchmark, ETF or trade-status classification.\n",
        encoding="utf-8",
    )
