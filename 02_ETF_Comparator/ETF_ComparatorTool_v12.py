...
#INPUT_FILENAME = Path("")
#INPUT_FILENAME = Path("D:/Tools/00_StockCodeMaster/03_ETF/17-07-NSE_ETF_Master_Library.csv")  # input file for ALL LATEST ETF tickers
INPUT_FILENAME = Path("D:/Tools/00_StockCodeMaster/03_ETF/17-07-NSE_ETF_Master_Library.csv")  # Default input file for tickers

#INPUT_FILENAME = None   
OUTPUT_FILENAME = Path("D:\\TMP\\ETF_Comparator\\17-07-ETF_Comparison-Report-All.xlsx")
CLASSIFICATION_FILENAME = Path("D:/Tools/StockCodeMaster/03_ETF/17-07-US_ETF_Classification_Mapping.csv")
print("***************************************************************************")
...
def compute_risk_metrics(etf_prices, benchmark_prices, risk_free_rate=0.04):
    if etf_prices is None or etf_prices.empty or benchmark_prices is None or benchmark_prices.empty:
        return {"Beta": None, "Alpha (Ann. %)": None, "Sharpe Ratio": None}

    df = pd.DataFrame({'ETF': etf_prices, 'Benchmark': benchmark_prices}).dropna()
    ...
    cov_matrix = np.cov(etf_ret, bench_ret)
    if cov_matrix[1, 1] == 0:
        return {"Beta": None, "Alpha (Ann. %)": None, "Sharpe Ratio": None}
        
    beta = cov_matrix[0, 1] / cov_matrix[1, 1]

    excess_etf = etf_ret - daily_rf
    excess_bench = bench_ret - daily_rf
    alpha_daily = excess_etf.mean() - beta * excess_bench.mean()
    alpha_ann = round(alpha_daily * 252 * 100, 2)
    
    ann_ret = etf_ret.mean() * 252
    ann_vol = etf_ret.std() * np.sqrt(252)
    
    sharpe = round((ann_ret - risk_free_rate) / ann_vol, 2) if ann_vol > 0 else None
...
def get_final_output_path(output_path, ticker_text, classification, mapping_provided_via_cli, output_provided_via_cli, input_provided_via_cli):
    ...
    if input_provided_via_cli:
        cli_parts.append("Inputfile")
...
def run(ticker_text=None, classification=None, mapping_path=None, output_path_arg=None, input_path_arg=None):
    base = Path(BASE_FOLDER)
    
    if input_path_arg:
        input_path = input_path_arg if input_path_arg.is_absolute() else base / input_path_arg
    else:
        input_path = INPUT_FILENAME if INPUT_FILENAME.is_absolute() else base / INPUT_FILENAME

    if mapping_path:
        actual_mapping_path = mapping_path if mapping_path.is_absolute() else base / mapping_path
    else:
        actual_mapping_path = CLASSIFICATION_FILENAME if CLASSIFICATION_FILENAME.is_absolute() else base / CLASSIFICATION_FILENAME
    ...
    input_provided_via_cli = input_path_arg is not None
    ...
    print("[INFO] Fetching Benchmark (SPY) for Risk Metrics calculation...")
    _, bench_prices, _ = get_price_series("SPY")
    ...
    for t in tickers:
        try:
            ...
            risk_metrics = compute_risk_metrics(prices, bench_prices)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare ETF returns, risk metrics, and fundamentals.")
    parser.add_argument(
        "--input",
        type=Path,
        default=None,
        help=f"Input CSV file containing ETF tickers. Default: {INPUT_FILENAME}",
    )
    parser.add_argument(
        "--tickers",
        default=None,
...
if __name__ == "__main__":
    args = parse_args()
    run(args.tickers, args.classification, args.classification_mapping_file, args.output, args.input)