...
INPUT_FILENAME = Path("D:/Tools/00_StockCodeMaster/03_ETF/22-07-US_ETF_Master_Library.csv")
#INPUT_FILENAME = Path("D:/Tools/00_StockCodeMaster/03_ETF/004/004_ETF_ALL_ENERGY.csv")
OUTPUT_FILENAME = Path("D:/TMP/ETF_Comparator/22-07-2026-All_USA-ETF_Comparator_Report-Revised.xlsx")
...
def find_column(columns, candidates):
    lookup = {str(col).strip().lstrip('\ufeff').strip('"\'').lower(): col for col in columns}
...
def load_instruments(input_path):
    ...
    # Fallback: if the header row itself was treated as data or parsed incorrectly,
    # check if the first row contains a valid ticker candidate or name.
    ...
    if ticker_col is None:
        # Check if the CSV has no proper header and the first row actually contains ticker symbols
        first_row_vals = [str(val).strip().upper() for val in df.iloc[0].values]
        for idx, val in enumerate(first_row_vals):
            if val in {c.upper() for c in TICKER_COLUMN_CANDIDATES}:
    ...
    if ticker_col is None:
        # If still not found, fallback to treating the very first column of the dataframe as the ticker column
        ticker_col = df.columns[0]
        print(f"[WARN] Ticker header not explicitly matched. Defaulting to first column: '{ticker_col}'")
...
def run(input_path, output_path, ticker_text=None):
    ...
    print("\n" + "=" * 88)
    print("STOCK / ETF PRICE COMPARATOR - POSTEXECUTION SUMMARY")
    print("=" * 88)
    ...
    print(f5 := f"Stock               : {(dataframe['ETF Flag'] == 'N').sum()}")
