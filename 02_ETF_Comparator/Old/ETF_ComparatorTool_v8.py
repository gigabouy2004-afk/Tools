...
#INPUT_FILENAME = Path("")
INPUT_FILENAME = Path("D:/Tools/StockCodeMaster/03_ETF/24-06-US_ETF_Master_Library-filtered.csv")   # Set to None or "" to skip file lookup
#INPUT_FILENAME = None   # Set to None or "" to skip file lookup
OUTPUT_FILENAME = Path("D:/TMP/24-06-ALL-USA_ETF_Filtered_Master_Comparison_Report.xlsx")
CLASSIFICATION_FILENAME = Path("D:/Tools/StockCodeMaster/03_ETF/24-06-US_ETF_Classification_Mapping.csv")
...
def collect_tickers(input_path, ticker_text=None):
    ...
    if cli_tickers:
        ...
        print(f"[INFO] Using CLI tickers: {len(tickers)} (File input bypassed)")
    else:
        file_tickers = load_tickers(input_path)
        tickers = sorted(set(file_tickers))
        print(f"[INFO] Using File tickers: {len(tickers)} (No CLI input provided)")
